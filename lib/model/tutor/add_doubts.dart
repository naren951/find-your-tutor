...
class AddDoubts extends StatefulWidget {
  ...
  currentuser=await FirebaseFirestore.instance.collection('users').doc(loggedInUser!.uid).get().then((value) => value.data());
  }
  ...
  text: "Post",
  onPressed: () async {
  doubts.add({"name":currentuser!['name'],"title":_titleController.text.toString().trim(),"description":_descriptionController.text.toString().trim()});
